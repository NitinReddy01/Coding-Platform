 ...
 * ```
 */
export const runCode = async (submission: Submission): Promise<SubmissionResponse> => {
  const response = await apiClient.post<SubmissionResponse>('/submissions/run', submission);
 ...
 * ```
 */
export const submitCode = async (submission: Submission): Promise<SubmissionResponse> => {
  const response = await apiClient.post<SubmissionResponse>('/submissions/submit', submission);
  return response.data;
};
